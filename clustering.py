# -*- coding: utf-8 -*-
"""
Created on Sun Mar 18 20:35:11 2018
...
class Clustering():
    ...
    '''def __entropy_graph(self, graph, clusid):
        ...
        for node in graph.graph:
            ...
            self.annotations[node][clusid] = self._entropy(len(graph.go_annotations[node]['gene']), \
                                          len(graph.go_annotations[self.goid]['gene']))
            if self.annotations[node][clusid] > maxm:
                maxm = self.annotations[node][clusid]
                
        if maxm == 0:
            return
        
        for node in graph.graph:
            self.annotations[node][clusid] = self.annotations[node][clusid] / maxm'''
